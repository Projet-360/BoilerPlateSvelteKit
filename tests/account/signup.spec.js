...
test.describe.serial('Password Reset Tests', () => {
	test.beforeAll(initializeBrowserAndPage);
	test.afterEach(closeBrowserAndPage);

	test('INVALID_EMAIL', async () => {
		await page.goto('http://localhost:5173/forgot-password', { waitUntil: 'networkidle' });

		// Assume the email field can be selected by its id 'email'
		await fillField(page, '#email', 'invalidemail@example.com');

		// Assume you click a button to submit the form
		await page.click('button[type="submit"]');

		// Check for an error message on the page
		const errorMessageElement = await page.waitForSelector('.notification');
		const errorMessageText = await errorMessageElement.textContent();
		expect(errorMessageText).toBe('Invalid email address X');
	});

	// Add more test cases like the above
});
